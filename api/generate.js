...
module.exports = async (req, res) => {
  ...
  try {
    ...
    // spikes — this is temporary, so we retry a couple of times with a
    // short delay before giving up, instead of failing immediately.
    ...
    for (let attempt = 1; attempt <= MAX_ATTEMPTS; attempt++) {
      ...
      if (!isOverloaded || isLastAttempt) {
        res.status(502).json({ error: 'Gemini API error', detail: lastErrText.slice(0, 500) });
        return;
      }

      // Wait a bit longer each retry (0.8s, then 1.6s) before trying again
